<template>
  <div class="container_large">
    ...
    <div class="login_holder">
      <h2>Kings Pickleball Ladder</h2>
       <h2 style="margin-bottom: 1rem">
                  Sign Up ({{ location_name }})
                </h2>
                <h3 style="margin-bottom: 2rem">
                  <a :href="other_location_url"
                    >Switch to {{ other_location_name }}</a
                  >
                  ?
                </h3>
      <p>
        To register for Kings’s Pickleball Ladder, please fill out the
        information below. After completing, the button at the bottom will take
        you to PayPal’s secure payment site where you can either log in to your
        existing PayPal account or pay with credit card.
      </p>
      <p><strong>$15 for singles, $10 for doubles by the deadline</strong></p>
      <p>
        <strong
          >$20 for singles, $15 for doubles after the deadline with additional
          ${{
            upcoming_season && upcoming_season.late_fee
              ? upcoming_season.late_fee
              : ""
          }}
          for late fee</strong
        >
      ...
      <p>
        ...
        <router-link :to="{ name: 'ratings' }">Ratings</router-link> page for
        reference
      </p>
                  ...
                  <div class="col-md-6">
                    <div class="form_group">
                      ...
                      <transition-group name="fade" v-if="errors">
                        <p
                          v-for="(error, keyIndex) in errors"
                          :key="keyIndex + 'from'"
                        >
                          <span v-if="error.password" style="color: #b22222">{{
                            error.password
                          }}</span>
                        </p>
                      </transition-group>
                  ...
                  <div class="col-md-6">
                    <div class="form_group">
                      ...
                      <select
                        ...
                        v-model="singles"
                        @change.prevent="
                          amountDue('Singles', 'singles', singles, 15)
                        "
                      >
                        <option value="">Select</option>
                        <option
                          v-for="data in single_rankings"
                          :key="data.id"
                          :value="data"
                        >
                          {{ data.title }} - $15.00
                  ...
                  <div class="col-md-4">
                    <div class="form_group">
                      ...
                      <select
                        ...
                        v-model="doubles"
                        @change.prevent="
                          amountDue('Doubles', 'doubles', doubles, 10)
                        "
                      >
                        <option value="">Select</option>
                        <option
                          v-for="data in double_rankings"
                          :key="data.id"
                          :value="data"
                        >
                          {{ data.title }} - $10.00
                ...
                <!-- <div class="row">
                  <div class="col-md-4">
                    <div class="form_group">
                      ...
                      <select
                        name=""
                        v-model="additional_mixed_doubles"
                        @change.prevent="
                          amountDue(
    ...
    <div class="terms_of_participation">
      ...
      <p>
        Participation in any/all pickleball ladders is at the player’s own risk.
        By registering on the website, you agree to the following Liability
        Release Statement:
      </p>
      <p>
        I do hereby agree to indemnify and hold King’s Pickleball Ladder, LLC,
        kingspickleballladder.com, from and against any and all liability for
        any injuries which may be incurred by me arising out of, or in any way
        connected with, my participation on the ladder.
      </p>
    </div>
...
/** prod env */
  ...
  methods: {
    ...
    async submit() {
      ...
      await userApis
        .requestUsers("post", this.signUpobj)
        .then((response) => {
  ...
  async created() {
    ...
    if (season) {
      ...
      this.is_deadline_passed = moment(new Date()).isSameOrBefore(
    ...
    if (this.single_rankings) {
      this.single_rankings = this.single_rankings.sort((a, b) =>
        a.title > b.title ? 1 : -1
      );
    }
    if (this.double_rankings) {
      this.double_rankings = this.double_rankings.sort((a, b) =>
        a.title > b.title ? 1 : -1
      );
    }
    if (this.mixed_rankings) {
      this.mixed_rankings = this.mixed_rankings.sort((a, b) =>
        a.title > b.title ? 1 : -1
      );
    }
    setTimeout(() => {
